    ...
    async runLoop(campaignId, signal) {
        try {
            while (!signal.aborted) {
                ...
                // EMAIL QUALITY FILTER - Skip obviously bad emails from poor name parsing
                const emailLocal = recipient.email.split('@')[0].toLowerCase();
                const badPrefixes = ['general.', 'manager.', 'director.', 'hotel.', 'hospitality.', 'ervaren.', 'seasoned.', 'cluster.', 'founder.', 'vice.', 'senior.', 'people-centric.', 'commercial.', 'brand.', 'high.', 'experience.', 'restaurant.', 'assistent.', 'nh.', 'van.', 'mercure.', 'avani.', 'accountmanager.'];
                const badSuffixes = ['.mba', '.rm', '.rt', '.phd'];
                const badPatterns = [
                    /^[a-z]+,\./,  // Starts with word followed by comma
                    /^\d/,  // Starts with number
                    /\.\d+@/,  // Has numbers before @
                    /^[a-z]{2,3}\./,  // Two-letter prefix (like "gm.")
                    /nederland@/,  // Location in email
                    /duitsland@/,
                    /belgie@/,
                    /zwitserland@/,
                    /frankrijk@/,
                ...
                const hasBadPrefix = badPrefixes.some(p => emailLocal.startsWith(p));
                const hasBadSuffix = badSuffixes.some(s => emailLocal.endsWith(s));
                const hasBadPattern = badPatterns.some(p => p.test(recipient.email.toLowerCase()));

                if (hasBadPrefix || hasBadSuffix || hasBadPattern) {
                    console.log(`[Runner] ⚠️ Skipping bad email format: ${recipient.email}`);
                    if (!campaigns[campaignIndex].logs) campaigns[campaignIndex].logs = [];
                    campaigns[campaignIndex].logs.unshift({
                        timestamp: new Date().toISOString(),
                        recipient: recipient.email,
                        status: 'bad_email',
                        error: 'Email appears to be from bad name parsing'
                    });
