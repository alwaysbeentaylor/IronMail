        ...
        try {
            while (!signal.aborted) {
                ...
                try {
                    ...
                    console.log(`[Runner] MX lookup failed for ${emailDomain}: ${mxErr.message}`);
                }

                if (!hasMX) {
                    console.log(`[Runner] ❌ Skipping ${recipient.email} - No valid MX records`);
                    // Log as skipped
                    ...
                    campaigns[campaignIndex].logs.unshift({
                        ...
                        status: 'mx_failed',
                        error: `No MX records for domain: ${emailDomain}`
                    });
