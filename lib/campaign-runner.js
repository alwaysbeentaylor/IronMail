    ...
    async runLoop(campaignId, signal) {
        try {
            while (!signal.aborted) {
                ...
                // ═══════════════════════════════════════════════════════════════
                console.log(`[Runner] 📧 SMTP verification for ${recipient.email}...`);
                ...
                try {
                    ...
                    if (validMailbox === false) {
                        mailboxExists = false;
                        console.log(`[Runner] ❌ SMTP: Mailbox does NOT exist - ${reason || 'rejected by server'}`);
                    } else if (validMailbox === true) {
                        console.log(`[Runner] ✅ SMTP: Mailbox verified!`);
                    } else {
                        // validMailbox is null = inconclusive (catch-all domain)
                        console.log(`[Runner] ⚠️ SMTP: Inconclusive (catch-all or greylist), proceeding...`);
                    }
                } catch (smtpErr) {
                    console.log(`[Runner] ⚠️ SMTP check failed: ${smtpErr.message}, proceeding anyway...`);
                    // Don't block on SMTP errors - some servers block verification
                }

                if (!mailboxExists) {
                    console.log(`[Runner] ❌ BLOCKED: ${recipient.email} - Mailbox doesn't exist`);
                    if (!campaigns[campaignIndex].logs) campaigns[campaignIndex].logs = [];
                    campaigns[campaignIndex].logs.unshift({
                        timestamp: new Date().toISOString(),
                        recipient: recipient.email,
                        status: 'invalid_mailbox',
                        error: 'SMTP verification failed - mailbox does not exist'
                    });
                    campaigns[campaignIndex].currentIndex = currentIndex + 1;
                    await writeData('campaigns', campaigns);
                    await delay(200);
