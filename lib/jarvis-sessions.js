// Jarvis Chat Sessions Management - BROWSER ONLY (localStorage)
...
export class JarvisSessions {
    static async getAllSessions() {
        try {
            const sessions = await browserStorage.read(SESSIONS_KEY) || [];
            return sessions;
        } catch (error) {
    ...
    static async getCurrentSessionId() {
        try {
            const sessionId = await browserStorage.read(CURRENT_SESSION_KEY);
    ...
    static async setCurrentSession(sessionId) {
        try {
            await browserStorage.write(CURRENT_SESSION_KEY, sessionId);
        } catch (error) {
    ...
    static async createNewSession(title = null) {
        ...
        const newSession = {
            ...
            title: title || `Chat ${now.toLocaleDateString('nl-NL', { timeZone: 'Europe/Amsterdam' })} ${now.toLocaleTimeString('nl-NL', { hour: '2-digit', minute: '2-digit', timeZone: 'Europe/Amsterdam' })}`,
            messages: [],
        ...
        await browserStorage.write(SESSIONS_KEY, trimmedSessions);
    ...
    static async updateSession(sessionId, updates) {
        ...
        await browserStorage.write(SESSIONS_KEY, sessions);
    ...
    static async addMessage(sessionId, message) {
        ...
        if (session.title.includes('Chat ') && session.messages.length === 1 && message.role === 'user') {
    ...
    static async deleteSession(sessionId) {
        ...
        await browserStorage.write(SESSIONS_KEY, filteredSessions);
